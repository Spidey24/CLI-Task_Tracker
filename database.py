...
def insert_todo(todo : Todo):
    ...
    with conn:
        ...
        {'task':todo.task,'category':todo.category,'date_added':todo.data_added,'date_completed':todo.date_completed,'status':todo.status,'position':todo.position})
...
def delete_todo(position):
    ...
    with conn:
        ...
        for ps in range(position+1,count):
            change_position(ps,ps-1,False)

def change_position(old_position:int,new_position:int,commit=True):
    c.execute('UPDATE todos SET position=:poisiton_new WHERE position=:position_old',
